...
# 高度函數
def f(x):
    # return -1*(x*x-2*x+1) #求 -x^2 + 2x - 1 的最高點，也就是 x^2 - 2x + 1 的最低點
    return -1*(x*x+3*x+5)   #求 -x^2 - 3x - 5 的最高點，也就是 x^2 + 3x + 5 的最低點
    # return -1*abs(x*x-4)  #求 -| x^2 - 4 | 的最高點，也就是 | x^2 - 4 | 的最低點
